...
class Defs:
    ...
    front_left_motor = Motor(
        port=0,
        inverted=False,
        calibration={"ticks_to_rad": 1.7616676383693458e-05, "vel_lpf_alpha": 1.0},
    )
    front_right_motor = Motor(
        port=1,
        inverted=True,
        calibration={"ticks_to_rad": 1.7920063643237234e-05, "vel_lpf_alpha": 1.0},
    )
    drum_motor = Motor(
        port=2, inverted=False, calibration={"ticks_to_rad": 1.0, "vel_lpf_alpha": 1.0}
    )
    servo_help_motor = Motor(
        port=3,
        inverted=False,
        calibration={"ticks_to_rad": 1e-05, "vel_lpf_alpha": 1.0},
    )
    drum_pusher_servo = ServoPreset(
        Servo(port=0), positions={"close": 138, "open": 0, "block_angle": 55}
    )
