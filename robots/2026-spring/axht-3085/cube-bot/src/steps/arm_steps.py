...
def grab_left_brown_cube_start_pos():
    return arm.move_angles(95, 85, -70, speed=100)  # rotate left to face correct direction
                # (giving base a bit of a turn so we don't hit the dor)
...
def drop_cube_into_container():
    return seq([
        arm.move_angles(CONTAINER_BASE_OFFSET, 100, 70, speed=150),  # move arm to drop cube into container position
